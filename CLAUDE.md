...
| 2 | 🔜 Active | dbt models, Great Expectations validation, anomaly alerts |
...
├── airflow/
│   ├── dags/
│   │   ├── ingestion_dag.py     # Main DAG: extract → load → validate → alerts
...
## What to Work on Next (Phase 2 remaining)

1. Add FRED ingestion task to `ingestion_dag.py` (extractor exists at `ingestion/extractors/fred.py` but not wired to the DAG yet).
2. Add a dbt task group to the DAG so dbt runs after raw load + validation.
3. Wire `validation/marts.py` into the DAG (`validate_fct_daily_returns`, `validate_fct_volatility`).
4. Add `schema.yml` files to dbt models with column-level tests.
5. Add `stg_macro` source definition to `sources.yml`.
